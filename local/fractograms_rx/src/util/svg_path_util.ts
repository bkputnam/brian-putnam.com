import { BORDER_RADIUS, BORDER_WIDTH, CELL_WIDTH_PX } from "./consts.ts";
 ...
 */
export function computeOutlinePath(
  ...
  const startingRow = 0;
  const startingCol = letterGrid[0].findIndex(
    (letter: string | null) => letter != null
  );
  ...
  do {
    if (exists(dir, counterClockwise(dir))) {
      ...
      pushCommands(line(dir, smallSide), corner(dir, clockwise(dir)));
      dir = clockwise(dir);
    }
