import { ScreenCoord } from "../data_structures/coords.js";
...
class DragDropService {
   ...
   */
  startDrag(id: number, targetEl: HTMLElement, startPos: ScreenCoord): boolean {
  ...
  endDrag(id: number, endPos: ScreenCoord) {
    ...
    const dropTargets = document
      .elementsFromPoint(endPos.x, endPos.y)
      .filter(
        (el: Element) =>
    ...
    for (const dropTarget of dropTargets) {
      ...
      if (dropTarget instanceof SVGSVGElement) {
        ...
        for (const innerDropTarget of innerDropTargets) {
          const containsDropPoint = boxContainsPoint(
...
function initGlobalMouseListeners() {
  const mouseDownListener = (e: MouseEvent) => {
    ...
    const dragStarted = ddService.startDrag(MOUSE_TOUCH_ID, targetEl, {
      x: e.clientX,
      y: e.clientY,
    });
...
function initGlobalTouchListeners() {
  ...
  const touchMoveListener = (e: TouchEvent) => {
    for (const touch of e.changedTouches) {
      ...
      ddService.moveDrag(touch.identifier, {
        x: touch.clientX,
        y: touch.clientY,
      });
    }
  ...
  const touchEndListener = (e: TouchEvent) => {
    for (const touch of e.changedTouches) {
      ...
      ddService.endDrag(touch.identifier, {
        x: touch.clientX,
        y: touch.clientY,
      });
