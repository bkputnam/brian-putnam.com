...
export class Solution {
  ...
  getSolutionText(colSeparator = "", rowSeparator = "\n"): string {
    return this.grid.map((row) => row.join(colSeparator)).join(rowSeparator);
  }
  ...
  private *leftoversToPieces(
    ...
    for (const coord of this.iterCoords()) {
      ...
      const pieceCoords = [...this.iterEmptySpaceCoords(placementBoard, coord)];
      const boundingBox = this.getBoundingBox(pieceCoords);
      const piece = Piece.emptyWithSize(boundingBox.width, boundingBox.height);
  ...
  private *iterEmptySpaceCoords(
    startingBoard: Board,
    startingCoord: BoardCoord
  ): Iterable<BoardCoord> {
    ...
    while (coordsToCheck.length > 0) {
      ...
      for (const neighbor of this.iterNeighbors(coord)) {
        if (
          !visitedCoords[neighbor.row][neighbor.col] &&
